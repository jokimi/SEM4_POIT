...
export const Game: React.FC<{}> = () => {
    let {
        ...
        setWon, won, setCellError, setShowError
    } = useSudokuContext();
    ...
    function _createNewGame(e?: React.ChangeEvent<HTMLSelectElement>) {
        let [temporaryInitArray, temporarySolvedArray] = getUniqueSudoku(e);
    ...
    function checkForSame(elems: string[]) {
        ...
        if (zeros > 0)
            return new Set(elems).size + zeros - 1 < 9;
    ...
    function getSquareIndexes(index: number) {
        ...
        indexesRow1 = indexesRow1.map((arr, index) => {
            if (index > 0) {
                return arr.map(ell => ell + 3)
            }
            return arr;
        })
        ...
        indexesRow2 = indexesRow2.map((arr, index) => {
            if (index > 0) {
                return arr.map(ell => ell + 3)
            }
            return arr;
        ...
        indexesRow3 = indexesRow3.map((arr, index) => {
            if (index > 0) {
                return arr.map(ell => ell + 3)
            }
            return arr;
