...
--insert into TEACHER values ('КЕМ', 'Козека Елизавета Максимовна', 'ж', 'ИСиТ')
select * from TEACHER
select * from TR_AUDIT;
...
use UNIVER
insert into TEACHER values ('ТЕСТ1', 'Для теста', 'м', 'ИСиТ')
select * from TR_AUDIT order by ID
...
go
delete from TEACHER where TEACHER = 'КЕМ'
insert into TEACHER values ('КЕМ', 'Козека Елизавета Максимовна', 'ж', 'ИСиТ')
select * from TEACHER
select * from TR_AUDIT order by ID
